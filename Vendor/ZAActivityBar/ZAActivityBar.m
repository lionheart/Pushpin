    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.alpha != 1.0f) {
            ...
            [UIView animateWithDuration:0.3
                             ...
                             animations:^{
                                 [self setYOffset:self.frame.size.height - (HEIGHT + YPADDING) - 7];
                                 self.alpha = 1;
...
- (void) positionBarOffscreen {
    [self setYOffset:self.frame.size.height + (HEIGHT + YPADDING)];
}
...
- (UIView *)barView {
    if(!barView) {
        CGRect rect = CGRectMake(XPADDING, FLT_MAX, self.frame.size.width, HEIGHT);
